...
void salvarArquivo(){
    ...
    fstream myFile;
    myFile.open("C:\\Users\\aleez\\Desktop\\faculdade\\Computação Grafica\\Projeto Codeblocks GLUT\\exercicio3008\\teste.txt", ios::out );
    myFile << points.size() << endl;
    for(int i = 0; i < points.size(); i++)
       myFile << points.at(i).x << " " << points.at(i).y << endl;
    myFile.close();
...
void abrirArquivo(){
    ...
    fstream myFile;
    myFile.open("C:\\Users\\aleez\\Desktop\\faculdade\\Computação Grafica\\Projeto Codeblocks GLUT\\exercicio3008\\teste.txt", ios::in);
    if(myFile.is_open()){
        esvaziaVector();
        myFile >> quantidadeLinhas;
        while(myFile >> x >> y){
            aux.x = x;
            aux.y = y;
